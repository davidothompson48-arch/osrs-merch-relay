...
def compact_portfolio(row):
    ...
    avg_one = (row.get("averages") or {}).get("1h") or {}
    avg_high_volume = avg_one.get("highPriceVolume")
    avg_low_volume = avg_one.get("lowPriceVolume")
    if isinstance(avg_high_volume, (int, float)) and isinstance(avg_low_volume, (int, float)):
        one_hour_volume = avg_high_volume + avg_low_volume
        fallback_high_share = avg_high_volume / one_hour_volume if one_hour_volume else None
    else:
        one_hour_volume = one.get("total")
        fallback_high_share = one.get("high") / one_hour_volume if one_hour_volume else None
    signal = row.get("exit_signal") or {}
    return {"item":row.get("item"),"item_id":row.get("item_id"),"quantity":row.get("quantity"),"basis_each":row.get("basis_each"),"basis_total":row.get("basis_total"),"current":row.get("current"),"freshness":row.get("freshness"),"1h":compact_avg(avg_one),"24h":compact_avg((row.get("averages") or {}).get("24h")),"movementPct":row.get("movementPct") or {},"highSideShare1h":vol.get("highSideShare1h") if "highSideShare1h" in vol else fallback_high_share,"oneHourVolume":one_hour_volume,"liquidity":row.get("liquidity"),"buy_limit":row.get("buy_limit"),"gross_pnl_high":row.get("gross_pnl_high"),"modeled_after_tax_pnl_high":row.get("modeled_after_tax_pnl_high"),"rating":row.get("rating"),"static_rating_before_exit_engine":row.get("static_rating_before_exit_engine"),"exit_action_required":signal.get("status")=="ACTION_REQUIRED","history_complete":row.get("history_complete"),"capital_aging":row.get("capital_aging")}


def compact_exit_engine(row):
    if not isinstance(row, dict):
        return {}
    return {k: row.get(k) for k in (
        "schema_version", "generated_at", "enabled", "rollout_status",
        "recommendation_authoritative", "market_data_ready", "data_status",
        "action_required", "actionable_count",
        "evaluated_positions", "data_gap_positions", "hold_for_use_exempt_items",
        "priority_actions", "precedence_rule", "thin_tape_rule", "rule"
    )}
...
def main():
    ...
    lite={
        "schema_version":8,"generated_at":p.get("generated_at"),"generated_unix":p.get("generated_unix"),"source":p.get("source"),"quality":p.get("quality"),
        ...
        "portfolio_exit_engine":compact_exit_engine(p.get("portfolio_exit_engine")),
        ...
        "deep_packet":"desk_packet.json","rule":"Routine desk runs should use this lite packet only. Check portfolio_exit_engine before all new deployment analysis. If action_required=true, report the exit as a MATERIAL CHANGE with its exact staged execution plan; its dynamic rating overrides any static runtime HOLD label. Thin tape changes execution style and sizing, not the existence of a validated sell signal. Then use the marginal-capital ladder for incremental GP, the absolute-velocity frontier for total GP/hour, the GE-slot-hour frontier for scarce slot allocation, and the balanced frontier for final risk-adjusted ranking. Fast flips require spread persistence before allocation. Profit hurdles, repeatability, attention modes and capital rotation remain staged unless their allocation_authoritative field is true. Fetch the deep packet solely for missing fields or a user-requested deep dive."
    }
