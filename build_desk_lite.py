...
def compact_profit_row(x):
    return {"engine":x.get("engine"),"id":x.get("id"),"name":x.get("name"),"strategy":x.get("strategy"),"profit_efficiency_score":x.get("profit_efficiency_score"),"expected_gp_per_hour":x.get("expected_gp_per_hour"),"expected_profit_at_capacity_gp":x.get("expected_profit_at_capacity_gp") if x.get("expected_profit_at_capacity_gp") is not None else x.get("player_time_adjusted_profit_gp"),"capacity_gp":x.get("capacity_gp"),"expected_after_tax_roi_pct":x.get("expected_after_tax_roi_pct"),"raw_after_tax_roi_pct":x.get("raw_after_tax_roi_pct"),"patient_roi_pct":x.get("patient_roi_pct"),"immediate_roi_pct":x.get("immediate_roi_pct"),"modeled_capacity_units":x.get("modeled_capacity_units"),"modeled_capacity_conversion_units":x.get("modeled_capacity_conversion_units"),"execution_probability_heuristic":x.get("execution_probability_heuristic"),"slippage_risk":x.get("slippage_risk"),"player_time_hours":x.get("player_time_hours"),"player_time_attention":x.get("player_time_attention"),"player_time_shadow_cost_gp":x.get("player_time_shadow_cost_gp"),"current_high":x.get("current_high"),"current_low":x.get("current_low"),"current_entry_reference":x.get("current_entry_reference"),"target_30d_or_7d_median":x.get("target_30d_or_7d_median"),"discount_to_30d_median_pct":x.get("discount_to_30d_median_pct"),"thirty_day_percentile":x.get("thirty_day_percentile"),"status":x.get("status"),"expected_hold_hours":x.get("expected_hold_hours"),"learning_adjustment_points":x.get("learning_adjustment_points")}
...
def main():
    ...
    lite={
        "schema_version":2,"generated_at":p.get("generated_at"),"generated_unix":p.get("generated_unix"),"source":p.get("source"),"quality":p.get("quality"),
        ...
        "profit_layer":{"objective":profit.get("objective"),"market_regime":profit.get("market_regime"),"capital_frontier":(profit.get("capital_frontier") or [])[:10],"fast_flip_capacity_velocity":[compact_profit_row(x) for x in (profit.get("fast_flip_capacity_velocity") or [])[:6]],"evergreen_capacity_velocity":[compact_profit_row(x) for x in (profit.get("evergreen_capacity_velocity") or [])[:6]],"conversion_capacity_velocity":[compact_profit_row(x) for x in (profit.get("conversion_capacity_velocity") or [])[:6]],"hard_value_set_edges":(profit.get("hard_value_set_edges") or [])[:6],"ge_slot_optimizer":profit.get("ge_slot_optimizer"),"shadow_learning":{"signals_total":(profit.get("shadow_learning") or {}).get("signals_total"),"graded_observations":(profit.get("shadow_learning") or {}).get("graded_observations"),"score_adjustments":(profit.get("shadow_learning") or {}).get("score_adjustments")},"player_time_shadow_value_gp_per_hour":profit.get("player_time_shadow_value_gp_per_hour"),"allocation_note":profit.get("allocation_note")},
        ...
        "deep_packet":"desk_packet.json","rule":"Routine desk runs should use this lite packet only. Profit rankings prioritize expected GP velocity/capacity/execution, not headline ROI. Fetch the deep packet solely for missing fields or a user-requested deep dive."
    }
